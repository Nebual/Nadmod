-- Nebual 2012-2016 (nebual@nebtown.info) presents:
-- NADMOD - Nebcorp Administration Designation Module
-- (a backronym for some reason)
...
if !NADMOD then
	...
	if not NADMOD.Groups[1] then NADMOD.Groups[1] = {Name="Guest",Color=Color(110,110,110),Permissions = {"Communications","Teleport","CanDealDamage"},Loadout = {"GravGun","PhysGun","ToolGun","Camera"}} end
	if table.Count(NADMOD.Groups) == 1 then
		NADMOD.Groups[4] = {
			Name="Respected",
			Color=Color(35,228,1),
			Permissions = {"Communications","Teleport","Weapons","TeleportOthers","CanDealDamage"},
			Loadout = {"GravGun","PhysGun","ToolGun","Camera","Crowbar","Pistol"},
		}
		NADMOD.Groups[25] = {
			Name="Admin",
	...
	local function MakeGman(pos)
		...
		Gman:SetOverlayText("("..NADMOD.Version..") I am Gman, god from the machine.")
		Gman.GetClass = function() return "Omnipresent" end Gman.UniqueID = function() return 666 end
		Gman.IsPlayer = function() return true end
		Gman.IsAdmin = function() return true end
		Gman.UserRank = 50
		Gman.CompareRank = 50
		Gman.UserID = function() return 666 end
		Gman.AddCount=function() return end
		Gman.AddCleanup=function() return end
		Gman.GetAimVector=function() return Vector() end
		Gman.GetShootPos=function() return Gman:GetPos() end
		Gman.CheckLimit = function() return true end
		Gman.GetWeapon = function() return NULL end
...
NADMOD.PermissionsAlias = {
	...
	Communications = {
		tell = NADMOD.Permissions.Communications.pm,
	},
	Annoys = {
...
hook.Add("PlayerInitialSpawn", "NADMOD_InitSpawn", function(ply)
	...
	if table.Count(NADMOD.Users) == 0 then
		timer.Create("NADMOD_GeneralHintInit"..ply:EntIndex(), 5, 1, function()
			NADMOD.Message("Welcome to NADMOD! To make yourself an admin, use your server console to run `! setrank [name] admin`, and then say `!menu` into chat.", ply)
		end)
	end
end)
...
function NADMOD.PlayerShouldTakeDamage(victim, attacker)
	return tobool(!attacker:IsValid() || !attacker:IsPlayer() || NADMOD.HasPermission(attacker, "CanDealDamage"))
end
...
function NADMOD.PlayerSpawnSWEP(ply, class, info)
	if (!NADMOD.HasPermission(ply, "CanDealDamage") and !NADMOD.HasPermission(ply, "Weapons")) and class != "laserpointer" then
...
print("[NADMOD - Nebcorp Administration Designation Module v"..NADMOD.Version.." Loaded]")
