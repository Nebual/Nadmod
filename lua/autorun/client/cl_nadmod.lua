...
net.Receive("nadmod_menu_download", function( len )
	...
	if decoded[1] == "groups" then
		...
		PermsPanel:SetSize(230, 400)
		PermsPanel:SetSpacing(10)
		PermsPanel:SetPadding(8)
		...
		WepsPanel:SetPos(870, 27)
		WepsPanel:SetSize(90, 400)
		WepsPanel:SetSpacing(10)
		WepsPanel:SetPadding(8)
		...
		button3.DoClick = function( button3 )
			...
			local function TestBoth()
				if temprank && tempname then
					-- Create a new rank based on the previous rank's permissions
					local previousRank = 0
					local previousTable = {Permissions = {}, Loadout = {"GravGun","PhysGun","ToolGun","Camera"}}
					for otherRank, otherTable in pairs(ret) do
						if otherRank < temprank and otherRank > previousRank then
							previousRank = otherRank
							previousTable = otherTable
						end
					end
					ret[temprank] = {Name = tempname, Color = Color(110,110,110), Permissions = table.Copy(previousTable.Permissions), Loadout = table.Copy(previousTable.Loadout)}
